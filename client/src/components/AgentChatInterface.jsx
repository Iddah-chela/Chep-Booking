...
const AgentChatInterface = ({ room, vacancyId, onClose, existingChatId }) => {
  ...
  const initializeChat = async () => {
    try {
      ...
      const { data } = await axios.post('/api/agent-chat/get-or-create', {
        vacancyId,
        roomDetails: {
      ...
      if (data.success) {
        setChat(data.chat);
        await axios.post('/api/agent-chat/mark-read', {
  ...
  const handleSendMessage = async (event) => {
    ...
    try {
      const token = await getToken();
      const { data } = await axios.post('/api/agent-chat/send', {
        chatId: chat._id,
        content: message
      }, {
        headers: { Authorization: `Bearer ${token}` }
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-2xl h-[600px] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b dark:border-gray-700">
          <div className="flex items-center gap-3">
            <img
              src={'https://ui-avatars.com/api/?name=Listing%20Agent&background=4f46e5&color=fff'}
              alt="Listing Agent"
        ...
        <div className="px-4 py-2 bg-gray-50 dark:bg-gray-700 border-b dark:border-gray-600">
          <p className="text-sm text-gray-600 dark:text-gray-300">
            About: <span className="font-medium">{listingTitle}</span> - {room?.buildingName || 'Main Building'} - {room?.roomType || 'Vacancy'}
          </p>
        </div>
