...
const PASS_PRICES = { '1day': 25, '7day': 150 };
...
export const initiateUnlock = async (req, res) => {
    try {
            ...
            return res.json({
                ...
                message: remaining > 0
                    ? `Free view ${priorCount + 1}/${FREE_UNLOCKS}! You have ${remaining} more free view${remaining > 1 ? 's' : ''} left.`
                            : `Last free view used. From your next property, choose Ksh 25/day or Ksh 150/week.`,
        ...
        if (!PASS_PRICES[passType]) {
            return res.json({ success: false, message: "Invalid pass type. Choose '1day' (Ksh 25) or '7day' (Ksh 150)." });
        }
