...
const PASS_PRICES = { '1day': 50, '7day': 200 };
...
export const initiateUnlock = async (req, res) => {
    try {
            ...
            return res.json({
                ...
                message: remaining > 0
                    ? `Free view ${priorCount + 1}/${FREE_UNLOCKS}! You have ${remaining} more free view${remaining > 1 ? 's' : ''} left.`
                            : `Last free view used. From your next property, choose Ksh 50/day or Ksh 200/week.`,
        ...
        if (!PASS_PRICES[passType]) {
            return res.json({ success: false, message: "Invalid pass type. Choose '1day' (Ksh 50) or '7day' (Ksh 200)." });
        }
